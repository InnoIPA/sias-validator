...
def do_inference(context, bindings, inputs, outputs, stream, batch_size=1):
    def execute_async():
        context.execute_async(batch_size=batch_size,
                              bindings=bindings, stream_handle=stream)
...
def get_engine(engine_path: str, trt_logger: Any):
    with open(engine_path, "rb") as f, trt.Runtime(trt_logger) as runtime:
        return runtime.deserialize_cuda_engine(f.read())

def get_engine_info(engine, log: bool=False) -> tuple:

    # Get the number of binding profiles (usually 1 for most engines)
    input_shapes, output_shapes = {}, {}

    # Loop through each binding profile to get input layer names and shapes
    for binding_index in range(engine.num_bindings):
        ...
        if engine.binding_is_input(binding_index):
            input_shapes[binding_name] = shape
        else:
            output_shapes[binding_name] = shape

    # Print input layer names and shapes
    if log:
